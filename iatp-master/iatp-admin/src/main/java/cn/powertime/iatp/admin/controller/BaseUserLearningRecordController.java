 ...
 * @since 2019-05-16
 */
@Api(value = "/base-user-learning-record",tags = {"用户学习记录表 前端控制器"})
@RestController
@RequestMapping("/base-user-learning-record")
public class BaseUserLearningRecordController extends BaseController {

    @Autowired
    private BaseUserLearningRecordFacade baseUserLearningRecordFacade;

    @ApiOperation(value = "用户收藏列表")
    @PostMapping("/list")
    public Object list(@Validated @RequestBody ParamPageVo<UserLearningRecordReqListVo> vo) {
        Page<UserLearningRecordRespListVo> respListVoPage = baseUserLearningRecordFacade.list(vo);
    ...
    public Object deleteBatch(@RequestBody List<Long> ids) {
        boolean result = baseUserLearningRecordFacade.delete(ids);
        if(result) {
            return success("删除成功");
        } else {
            return error("删除失败");
