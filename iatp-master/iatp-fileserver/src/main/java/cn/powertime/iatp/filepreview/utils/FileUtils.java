...
import cn.powertime.iatp.filepreview.model.FileType;
...
public class FileUtils {
    ...
    public FileAttribute getFileAttribute(String uri) {
        ...
        if(StringUtils.isBlank(suffix) || StringUtils.isBlank(baseName)) {
            log.error("传入的文件名称错误，name: {}",uri);
            throw new FileServerException("传入的文件名称错误！");
        }

        SysFile sysFile = sysFileService.getById(baseName);
        if(sysFile == null) {
            log.error("通过名称未查询到相关数据，name: {}",uri);
            throw new FileServerException("通过名称未查询到相关数据！");
        }
        ...
        // 路径转码
        FileType type = typeFromUrl(suffix);
        return new FileAttribute(type,suffix,sysFile.getOrigName(),baseName,uri,(filePath + File.separator + fileName));
    }
}
